...
double estimate_memory_roofline() {
  ...
  res_t result = run_rw_roofline<FLOPS_PER_ELEM>(/* allowed time: */ 5, a);
  free(a);

  return (WSS_BYTES * 2.0) / result.min_time / 1e9;
}

double estimate_max_flops_roofline() {
  ...
  data_t* a = (data_t*)hs_alloc(WSS_BYTES);
  init_const(a, WSS_ELEMS, 1.0);
  res_t result = run_rw_roofline<FLOPS_PER_ELEM>(/* allowed time: */ 5, a); // Low AI (flops per double) to test memory bound
  free(a);
...
int main() {
  ...
  double max_flops_estimate = estimate_max_flops_roofline();
